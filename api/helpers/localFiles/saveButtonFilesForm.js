...
const handleFile = (
  ...
  const isAwaited = Object.values(awaitingButtons).some(
    (button) =>
...
const saveButtonFilesForm = (req, projectId, awaitingButtons) => {
  return new Promise(async (resolve, reject) => {
    ...
    form.on("file", (...props) =>
      handleFile(projectId, awaitingButtons, filePromises, ...props)
    );
    form.on("error", (err) => {
      return reject(err);
    });
    ...
    form.on("finish", async () => {
      ...
      return resolve({
        ...
        successfulCount:
          ongoingDataUpdate.buttonFiles.successfulFilenames.length,
      });
    });
